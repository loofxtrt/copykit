...
def wrap_make_symlinks(pack_root: Path, symlinks_dir: Path, entry_key: str = None, aliases: list[str] = None):
    ...
    if entry_key:
        svg_name = normalize_svg_name(entry_key)
    elif aliases:
        svg_name = normalize_svg_name(aliases[0])
    else:
        logger.error("ao menos um nome de chave ou alias deve ser passado")
    
    main_icon = target_dir / svg_name
    create.create(
        target_path=target_dir / main_icon,
        file_to_create=entry_key["substitute"]
    )
    ...
    for key, entry in rep_map.items():
        ...
        for dest in destinations:
            ...
            if make_symlinks_at is not None:
                wrap_make_symlinks(dest, make_symlinks_at, ignore_key)
                continue
